...
def generate_fresh(event, context):
    ...
    try:
        ...
        orchestrator = AgentOrchestrator(CURIO_TABLE)
        
        # Start the agent orchestration process
        orchestrator.update_agent_status(run_id, "NEWS_FETCHER", "STARTING")
        
        # For demo purposes, we'll simulate the process
        # In production, this would be async (Step Functions or SQS)
        try:
            # Simulate News Fetcher Agent
            orchestrator.update_agent_status(run_id, "NEWS_FETCHER", "RUNNING")
            ...
            mock_news = [
                {
                    ...
                    "category": "TECHNOLOGY",
                    "summary": "Researchers unveil a revolutionary AI system that demonstrates unprecedented reasoning capabilities.",
                    "relevance_score": 0.95
                },
                {
                    "title": "Gen Z Leads Climate Action with Innovative Carbon Capture Startup",
                    "category": "ENVIRONMENT", 
                    "summary": "Young entrepreneurs develop breakthrough technology to remove CO2 from atmosphere.",
                    "relevance_score": 0.92
                },
                {
                    "title": "TikTok's New Feature Changes How We Share Memories",
                    "category": "CULTURE",
                    "summary": "Social media platform introduces AI-powered memory curation that's going viral.",
                    "relevance_score": 0.88
                }
            ]
            
            orchestrator.update_agent_status(run_id, "CONTENT_CURATOR", "RUNNING")
            time.sleep(1)
            
            orchestrator.update_agent_status(run_id, "SCRIPT_GENERATOR", "RUNNING") 
            time.sleep(1)
            ...
            final_content = {
                ...
                'script': 'Breaking news! Our AI agents have curated the most engaging stories for you today...',
        ...
        print(f"Generate fresh error: {e}")
        return {
            'statusCode': 500,
            'headers': cors_headers(),
            'body': json.dumps({'error': str(e)})
...
def agent_status(event, context):
    ...
    try:
        run_id = event.get('queryStringParameters', {}).get('runId')
        
        if not run_id:
            return {
                'statusCode': 400,
                'headers': cors_headers(),
                'body': json.dumps({'error': 'Missing runId parameter'})
            }
        
        # Get agent status from DynamoDB
        response = dynamodb.get_item(
            TableName=CURIO_TABLE,
            Key={'pk': {'S': f'generation#{run_id}'}}
        )
        ...
        if 'Item' in response:
            ...
            return {
                ...
                'body': json.dumps({
                    'runId': run_id,
                    'currentAgent': item.get('currentAgent', {}).get('S', 'UNKNOWN'),
            ...
            return {
                'statusCode': 404,
                'headers': cors_headers(),
                'body': json.dumps({'error': 'Run ID not found'})
            }
